...
Imaging
ImagingGetBand(Imaging imIn, int band, int device = 0) {
    Imaging imOut;
    int x, y;
...
int
ImagingSplit(Imaging imIn, Imaging bands[4], int device=0) {
    int i, j, x, y;
...
Imaging
ImagingPutBand(Imaging imOut, Imaging imIn, int band, int device = 0) {
    int x, y;
...
Imaging
ImagingFillBand(Imaging imOut, int band, int color, int device) {
    int x, y;
...
ImagingMerge(const char *mode, Imaging bands[4]) {
    ...
    imOut = ImagingNewDirty(mode, firstBand->xsize, firstBand->ysize, int device = 0);
    if (!imOut) {
        return NULL;
    }
